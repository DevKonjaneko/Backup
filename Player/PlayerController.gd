extends CharacterBody3D
# Health
@export var health: float = 100.0
@export var max_health: float = 100.0
# Stamina
var can_sprint: bool = true
@export var stamina: float = 100.0
@export var max_stamina: float =100.0
@export var stamina_drain_rate: float = 25.0 # Stamina ที่ใช้ต่อวินาทีขณะวิ่ง
@export var stamina_regen_rate: float = 10.0 # Stamina ที่ฟื้นฟูต่อวินาทีเมื่อไม่วิ่ง
@export var stamina_bar_path: NodePath
@onready var stamina_bar: ProgressBar = get_node(stamina_bar_path)
# Movement
@export var crouch_speed: float = 1.25
@export var speed: float = 2.25
@export var sprint_speed: float = 4.00
@export var jump_velocity: float = 5.00
# SoundEffect
@onready var heartbeat: AudioStreamPlayer = $Heartbeat
@onready var footstep_walk = $Footstep_Walk
@onready var footstep_sprint= $Footstep_Sprint
# Death Screen
@onready var death_screen = $player_ui/PlayerUI/Death_screen
# Crouching
var is_crouching = false
# Heartbeat
var is_heartbeat = false

func _ready() -> void:
	# Lock and Hide Cursor
	Input.mouse_mode = Input.MOUSE_MODE_CAPTURED
	# Set Stamina Bar
	if stamina_bar:
		stamina_bar.max_value = max_stamina
	# Hide Death Screen
	if death_screen:
		death_screen.hide()
	
func _process(_delta: float) -> void:

	update_heartbeat()

	if health <= 0:
		player_died()
	
	if Input.is_action_just_pressed("Crouch"):
		is_crouching = !is_crouching
		print("is_crouch == true")
		
func handle_crouching(_delta: float) -> void:
	if is_crouching and $CollisionShape3D.shape.height > 0.2:
		var crouch_height = lerp($CollisionShape3D.shape.height, 0.2, 0.2) 
		$CollisionShape3D.shape.height = crouch_height
		
	if !is_crouching and $CollisionShape3D.shape.height < 2.0:
		var crouch_height = lerp($CollisionShape3D.shape.height, 2.0, 0.2)
		$CollisionShape3D.shape.height = crouch_height
	
func _physics_process(delta: float) -> void:
	#เช็ค Pause ก่อนทำอะไร!
	if (Input.mouse_mode == Input.MOUSE_MODE_VISIBLE):
		return
	handle_crouching(delta)
	handle_movement(delta)
	handle_gravity(delta)
	move_and_slide()
	
	if stamina_bar:
		stamina_bar.value = stamina

func handle_stamina(delta: float, is_sprinting: bool, is_moving: bool) -> void:
	if is_sprinting and is_moving:
		stamina -= stamina_drain_rate * delta
	elif not is_sprinting and stamina < max_stamina:
		stamina += stamina_regen_rate * delta
	stamina = clamp(stamina, 0.0, max_stamina)
	
	if stamina <= 0:
		can_sprint = false
	elif stamina >= 10:
		can_sprint = true

func handle_movement(delta):
	# Check is_moving?
	var input_dir := Input.get_vector("M_Left", "M_Right", "M_Forward", "M_Backward")
	var is_moving = not input_dir.is_zero_approx()
	# Choose Speed
	var current_speed: float
	if is_crouching:
		current_speed = crouch_speed
	elif Input.is_action_pressed("Sprint") and can_sprint and is_moving:
		current_speed = sprint_speed
	else:
		current_speed = speed
	
	var is_sprinting = (current_speed == sprint_speed) and is_moving
	
	var direction := (transform.basis * Vector3(input_dir.x, 0, input_dir.y)).normalized()
	if direction:
		velocity.x = direction.x * current_speed
		velocity.z = direction.z * current_speed
	else:
		velocity.x = move_toward(velocity.x, 0, speed)
		velocity.z = move_toward(velocity.z, 0, speed)
	
	# Manage Stamina
	handle_stamina(delta, is_sprinting, is_moving)
		
	#Footstep Logic
	var is_moving_on_floor = is_moving and is_on_floor()
	if is_moving_on_floor and is_sprinting:
			if not footstep_sprint.playing:
				footstep_sprint.play()
			footstep_walk.stop()
	elif is_moving_on_floor:
			if not footstep_walk.playing:
				footstep_walk.play()
			footstep_sprint.stop()
	else:
		footstep_walk.stop()
		footstep_sprint.stop()

func handle_gravity(delta):
	if not is_on_floor():
		velocity += get_gravity() * delta
	if Input.is_action_just_pressed("Jump") and is_on_floor():
		velocity.y = jump_velocity
		
func update_heartbeat() -> void:
	if not heartbeat or not heartbeat.stream:
		return
	# คำนวณ % HP
	var health_percent = (health / max_health) * 100.0
	# ⭐ กรณีที่ 1: HP น้อยกว่า 30% → เสียงหัวใจเต้นเร็ว
	if health_percent < 30.0:
		if not is_heartbeat:
			heartbeat.play()
			is_heartbeat = true
		# เล่นเร็วขึ้นเมื่อ HP ต่ำมาก
		heartbeat.pitch_scale = 1.5 if health_percent < 15.0 else 1.2
		heartbeat.volume_db = -10.0  # เสียงดังขึ้น
	# ⭐ กรณีที่ 2: HP ระหว่าง 30-50% → เสียงหัวใจเต้นช้า
	elif health_percent < 50.0:
		if not is_heartbeat:
			heartbeat.play()
			is_heartbeat = true
		heartbeat.pitch_scale = 1.0
		heartbeat.volume_db = -15.0  # เสียงเบาลง
	# ⭐ กรณีที่ 3: HP มากกว่า 50% → ปิดเสียง
	else:
		if is_heartbeat:
			heartbeat.stop()
			is_heartbeat = false
			
#⭐Healsystem
func heal(amount: float) -> void:
	health += amount
	health = clamp(health, 0.0, max_health)
	print("💚 HP: ", health, "/", max_health)

func take_damage(amount: float) -> void:
	health -= amount
	health = clamp(health, 0.0, max_health) # clamp() ป้องกันไม่ให้เลือดติดลบ
	print("💥 โดนดาเมจ! ", amount, " หน่วย, HP เหลือ: ", health, "/", max_health)

#⭐Died
func player_died() -> void:
	print("💀 Player Died!")
	# Stop HeartBeat
	if heartbeat:
		heartbeat.stop()
	# Show DeathScreen
	if death_screen:
		death_screen.show()
	# Show Cursor
	Input.mouse_mode = Input.MOUSE_MODE_VISIBLE
	# เพิ่มโค้ดตายที่นี่
